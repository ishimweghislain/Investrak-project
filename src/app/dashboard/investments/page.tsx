        ...
        <div className="min-h-screen bg-[#0a0c10] text-white p-6 pb-24 md:pb-10 md:pt-10">
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    {investments.map((inv) => (
                        <div key={inv.id} className="bg-[#161b22]/60 backdrop-blur-xl p-6 rounded-[24px] border border-white/5 hover:border-blue-500/20 transition-all group">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <h3 className="font-bold text-xl text-white mb-1 group-hover:text-blue-400 transition-colors">{inv.title}</h3>
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between items-center p-3 bg-[#0d1117] rounded-xl border border-white/5">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-lg bg-blue-600/10 flex items-center justify-center text-blue-400">
                                            <DollarSign className="w-4 h-4" />
                                        </div>
                                        <span className="text-sm font-medium text-slate-300">Principal</span>
                                    </div>
                                    <span className="font-bold text-white text-lg">${inv.amount.toLocaleString()}</span>
                                </div>

                                <div className="flex justify-between items-center p-3 bg-[#0d1117] rounded-xl border border-white/5">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-lg bg-green-600/10 flex items-center justify-center text-green-400">
                                            <TrendingUp className="w-4 h-4" />
                                        </div>
                                        <span className="text-sm font-medium text-slate-300">ROI Target</span>
                                    </div>
                                    <span className="font-bold text-green-400 text-lg">{inv.roi ? `${inv.roi}%` : 'TBD'}</span>
                                </div>
                                ...
                                {inv.maturityDate && (
                                    <div className="flex justify-between items-center p-3 bg-[#0d1117] rounded-xl border border-white/5">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-sm font-medium text-slate-300">Maturity</span>
                                        </div>
                                        <span className="font-bold text-white">{new Date(inv.maturityDate).toLocaleDateString()}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                {investments.length === 0 && (
                    <div className="text-center py-20 bg-[#161b22]/40 rounded-[32px] border border-white/5">
                        <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-4">
                            <TrendingUp className="w-8 h-8 text-slate-600" />
                        </div>
                        <h3 className="text-xl font-bold text-white mb-2">No Active Holdings</h3>
